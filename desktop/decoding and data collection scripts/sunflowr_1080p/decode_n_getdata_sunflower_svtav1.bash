#!/bin/bash
# SUNFLOWER - SVT_AV1 - DECODING AND GET DATA
...
output_file="sunflower_1080p.csv"
...
# Decoding both VBR and CQP mode

decode_n_getdata_vbr(){
# DECODING
...
/usr/bin/time -o ${dest_dir}/dectime_gav1.txt -f "%U %S %E" ${gav1_path} ${dest_dir}/${seq_name}_${i}.${codec}
...
do
# DECODING
(/usr/bin/time -o ${dest_dir}/dectime_q${i}.txt -f "%U %S %E" ffmpeg -y -i ${dest_dir}/${seq_name}_q${i}.${codec} -vcodec rawvideo -f rawvideo ${dest_dir}/${seq_name}_q${i}.yuv)
# GET BITRATE
bitrate=`grep 'kbps' ${dest_dir}/encdata_q${i}.txt | tail -1 | awk '{print $5}'`
# GET ENCODING TIME
enctime=`./get_time ${dest_dir}/enctime_q${i}.txt min | grep 'User+System_time' | awk -F ':' '{print $2}'`
wallclock=`./get_time ${dest_dir}/enctime_q${i}.txt min | grep 'Wall_clock' | awk -F ':' '{print $2}'`
# GET DECODING TIME
dectime=`./get_time ${dest_dir}/dectime_q${i}.txt sec | grep 'User+System_time' | awk -F ':' '{print $2}'`
# GET PSNR
psnr=`./psnr_video ${org_seq_name} ${dest_dir}/${seq_name}_q${i}.yuv ${w} ${h} ${n}`
# GET VMAF
cd ${vmaf_dir}
...
yuv420p ${w} ${h} \
${curr_dir}/${org_seq_name} \
${curr_dir}/${dest_dir}/${seq_name}_q${i}.yuv \
--out-fmt json) &> ${curr_dir}/vmaf.json

cd ${curr_dir}
vmaf=`tail -4 vmaf.json | grep 'score' | awk -F ': ' '{print $2}' | sed -r 's/.{13}$//'`
# GET DAV1D DECODING TIME
(/usr/bin/time -o ${dest_dir}/dectime_dav1d.txt -f "%U %S %E" ${dav1d_path} -i ${dest_dir}/${seq_name}_q${i}.${codec} -o /dev/null)
dav1d_dectime=`./get_time ${dest_dir}/dectime_dav1d.txt sec | grep 'User+System_time' | awk -F ':' '{print $2}'`
# GET SVT-AV1 DECODING TIME
cd ${svtav1_path}

/usr/bin/time -o ${curr_dir}/${dest_dir}/dectime_svtav1.txt -f "%U %S %E" ./SvtAv1DecApp -i ${curr_dir}/${dest_dir}/${seq_name}_q${i}.${codec} -o /dev/null

cd ${curr_dir}

svtav1_dectime=`./get_time ${dest_dir}/dectime_svtav1.txt sec | grep 'User+System_time' | awk -F ':' '{print $2}'`
# GET GAV1 DECODING TIME
/usr/bin/time -o ${dest_dir}/dectime_gav1.txt -f "%U %S %E" ${gav1_path} ${dest_dir}/${seq_name}_q${i}.${codec}
gav1_dectime=`./get_time ${dest_dir}/dectime_gav1.txt sec | grep 'User+System_time' | awk -F ':' '{print $2}'`
# WRITE DATA
echo "${encoder_name};${bitrate};${psnr};${vmaf};${wallclock};${enctime};${dectime};${dav1d_dectime};${svtav1_dectime};${gav1_dectime}" >> ${output_file}
done
